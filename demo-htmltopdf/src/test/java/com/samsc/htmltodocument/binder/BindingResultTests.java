...
/**
 * Tests for {@link BindingResult}.
 *
 * @author Andy Lian
 */
class BindingResultTests {
  ...
  void writeValue() {
    runner.run(
        (context) -> {
          ...
          final BindingResult object = new BindingResult();
  ...
  void readValue() {
    runner.run(
        (context) -> {
          ...
          final BindingResult object =
              objectMapper.readValue("{ \"content\":\"abc\" }", BindingResult.class);
